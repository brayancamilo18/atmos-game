(() => {
    ...
    async function loadMyScores() {
        ...
        try {
            const res = await apiFetch(
                `/my-scores?game=${encodeURIComponent(GAME_NAME)}`,
            );
            if (!res.ok) return;
    ...
    // CAMBIO: ahora hay 10 capas (antes eran 6)
    function getLayerByHeight(h) {
        // 10 capas (en metros). Ajusta los límites si quieres que el juego cambie antes/después.
        // 0) 0 - 1499
        if (h < 1500) {
            return {
                name: "Cielo bajo",
    ...
    function getAllowedObstacleTypes(height, layerObstacleTypes) {
        ...
        if (height < 9000) {
            return layerObstacleTypes.filter(
                (t) => t === "bird" || t === "drone",
            );
        }
    ...
    function spawnPlatformsIfNeeded() {
        ...
        while (highestY > game.cameraY - 900) {
            ...
            game.platforms.push(
                createPlatform(clamp(x, 12, W - 100), highestY, type),
            );
            ...
            if (canSpawnObstacleFromPlatforms) {
                ...
                const allowedTypes = getAllowedObstacleTypes(
                    height,
                    layer.obstacleTypes,
                );
    ...
    function updatePlayer(layer) {
        ...
        if (layer.wind !== 0) {
            p.x +=
                Math.sin(performance.now() * 0.0012 + p.y * 0.01) * layer.wind;
        }
        ...
        if (p.y < H * 0.42 + game.cameraY) {
            ...
            game.bestHeight = Math.max(
                game.bestHeight,
                Math.floor(game.worldHeight),
            );
    ...
    function drawStartOverlay() {
        ...
        ctx.font = "16px Arial";
        ctx.fillText(
            "Sube sin fin por las capas de la atmósfera",
            W / 2,
            H / 2 - 40,
        );
